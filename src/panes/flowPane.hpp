...
namespace NLUI {
    ...
    private:
        ...
        // TODO these wouldn't be needed if caching values was done
        std::vector<std::vector<std::shared_ptr<Component>>> rows;
        std::vector<std::vector<std::shared_ptr<Component>>> cols;
    ...
    protected:
        FlowPane(const bool horizontal, const glm::ivec2 &minSize, const glm::ivec2 &maxSize);

    public:
        virtual ~FlowPane() override;

        static std::shared_ptr<FlowPane> create(const bool horizontal = true, const glm::ivec2 &minSize = glm::ivec2(0, 0), const glm::ivec2 &maxSize = glm::ivec2(INT_MAX, INT_MAX));
